...
usage(int exit_status)
{
  ...
  printf("  -s : Sign (pos, neg, rnd, default: NONE (keep original sign))\n");
  printf("  -h : Print this help message\n");
  exit(exit_status);
}

static void
print_array(int* array, size_t length)
{
  printf("%lu\n", length);
...
static enum sign_type
parse_sign(char* sign)
{
  if (strcmp(sign, "pos") == 0) {
    return POS;
  } else if (strcmp(sign, "neg") == 0) {
    return NEG;
  } else if (strcmp(sign, "rnd") == 0) {
    return RND;
  }
...
static void
parse_args(int argc, char* argv[], struct array_config* config)
{
  int c;

  *config = (struct array_config){
    .length = 10, .min = 1, .max = 100, .order = UNS, .sign = NONE
  };
