...
const char* ALG_CHOICES = "binsort,heapsort,introsort,isort,mergesort,quicksort,"
                          "quicksort_std,selection_sort,shellsort,timsort";
...
static void
parse_args(int argc, char* argv[], struct workbench* wb)
{
  ...
  while ((c = getopt_long(argc, argv, "a:t:f:s:dh", long_options, &option_index)) !=
         -1) {
    switch (c) {
      case 'a':
